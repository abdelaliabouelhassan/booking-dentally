    ...
    <template v-slot:body>
      <div class="p-10 space-y-8" v-if="!loading">
        ...
        <ul class="divide-y divide-theme-gray-light">
          <li v-for="(Dentist,index,key) in Dentists" :key="key" v-show="Dentist.id == 14615 || Dentist.id == 44633" class="  hover:px-2" :class="{'px-2':store.practitioner.id  == Dentist.id}" @click="SelectDentist(Dentist)">
            <a href="javascript:void(0)" class="flex items-center py-4">
              ...
              <div class="flex flex-col leading-tight mr-2">
                <strong class="text-black">{{Dentist.user.title == 'Dr' ? 'Dr' : ''}} {{Dentist.user.first_name}} {{Dentist.user.middle_name}} {{Dentist.user.last_name}}</strong>
                <span class="text-theme-gray text-sm">{{Dentist.user.role}}</span>
              </div>
...
export default {
  ...
  methods:{
    SelectDentist(Dentist){
      ...
      this.store.practitioner.name = Dentist.user.title == 'Dr' ? 'Dr ' +  Dentist.user.first_name  : '' + Dentist.user.first_name ;
