    ...
    <template v-slot:body>
      <div class="p-10 space-y-8" v-if="!loading">
        ...
        <ul class="grid gap-4 grid-cols-1 text-center leading-tight">
          <li v-for="(Treatment,index,key) in Treatments" :key="key" v-show="Treatment.id == 144000"  >
            <a
              ...
              "
              :class="{'border-theme-gold':store.treatments.id == Treatment.id}"
            >
     ...
     <template v-slot:pagination>
          <p class="text-theme-gray-light">1 of 4</p>
    </template>  
     ...
     <template v-slot:buttons>
        <div class="space-x-4 flex flex-row ">
                ...
                <a @click="LoadMore"  v-if="false" href="javascript:void(0)" class="bg-theme-gold text-white rounded-full p-4 w-full ">Load More</a>
            </div>
    </template>
  </MainLayouts>
</template>
